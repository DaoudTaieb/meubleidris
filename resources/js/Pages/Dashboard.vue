            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12">
                <div class="bg-white p-6 rounded-[2rem] border border-[#e3e3e0] shadow-sm flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-black text-brand-gold tabular-nums">{{ formatCurrency(fournisseurs.reduce((acc, f) => acc + (parseFloat(f.solde) > 0 ? parseFloat(f.solde) : 0), 0)) }} TND</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-[#e3e3e0] shadow-sm overflow-hidden">
                <div class="p-6 border-b border-[#f0f0f0] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="relative w-full sm:w-auto">
                        ...
                        <input type="text" placeholder="Rechercher..." class="w-full bg-[#f8f8f7] border-transparent rounded-xl py-2 pl-10 pr-4 text-sm focus:bg-white focus:border-brand-gold focus:ring-0 transition-all outline-none sm:w-64" />
                    </div>
                </div>
                            ...
                            <tr v-for="f in fournisseurs" :key="f.fournisseurid" class="hover:bg-[#fcfcfb] group transition-colors">
                                ...
                                <td class="px-4 md:px-8 py-5 md:py-6">
                                    <div class="flex flex-col min-w-0">
                                        <span class="font-bold text-[#1b1b18] group-hover:text-brand-gold transition-colors leading-tight text-sm md:text-base truncate max-w-[120px] sm:max-w-none">{{ f.nom }}</span>
                                ...
                                <td class="px-4 md:px-8 py-5 md:py-6 text-right">
                                    <span :class="[parseFloat(f.solde) > 0 ? 'text-brand-gold' : parseFloat(f.solde) < 0 ? 'text-[#10b981]' : 'text-[#f59e0b]', 'font-bold text-sm md:text-lg tabular-nums tracking-tighter']">
                                        {{ formatCurrency(f.solde) }}
                                    </span>
                                </td>
                                <td class="px-4 md:px-8 py-5 md:py-6 text-right">
                                    <Link :href="`/fournisseur/${f.fournisseurid}/releve`" class="inline-flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-xl bg-[#f8f8f7] text-[#1b1b18] hover:bg-brand-gold hover:text-white transition-all shadow-sm">
